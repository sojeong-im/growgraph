        ...
        <section className="min-h-screen relative overflow-hidden bg-black flex flex-col justify-center">
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-6 py-20">
                <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-white/80 text-lg md:text-xl font-light max-w-2xl mx-auto mb-12 animate-fade-in-up delay-200" style={{ wordBreak: 'keep-all' }}>
                        느낌이 데이터가 되는 순간, VIBE<br />
                        <span className="text-white/50 text-base mt-2 block">마케터가 설계한 '스타일 원형' 분석 시스템</span>
                    </p>
                    <Link
                        to="/services"
                        className="inline-block px-12 py-5 border border-white/50 text-white text-sm tracking-[0.2em] uppercase hover:bg-white hover:text-black transition-all duration-500 animate-fade-in-up delay-300"
                    >
                        시작하기
                    </Link>
                </div>
    ...
    return (
        <section className="py-32 bg-white">
            <div className="max-w-4xl mx-auto px-6 text-center reveal">
                <p className="text-xs tracking-[0.3em] uppercase text-gray-400 mb-8">Brand Philosophy</p>
                ...
                <div className="w-px h-20 bg-black mx-auto mb-12" />
        ...
        <section id="process" className="py-24 bg-gray-50 border-y border-gray-200">
            <div className="max-w-7xl mx-auto px-6">
                <div className="text-center mb-20 reveal">
                    <p className="text-xs tracking-[0.3em] uppercase text-gray-400 mb-4">Process</p>
                    <h2 className="font-serif text-4xl md:text-5xl font-light">VIBE SYSTEM</h2>
                    <p className="text-gray-500 mt-4">데이터 분석 체계</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
                    {/* Connecting Line for Desktop */}
                    <div className="absolute top-[15px] left-0 w-full h-px bg-gray-200 hidden md:block" />

                    {steps.map((step, idx) => (
                        <div key={idx} className="relative bg-gray-50 pt-8 md:pt-12 reveal delay-100">
                            {/* Dot */}
                            <div className="absolute top-0 left-0 md:left-1/2 md:-translate-x-1/2 w-8 h-8 md:w-3 md:h-3 rounded-full bg-white border border-black flex items-center justify-center md:block">
                                <span className="md:hidden text-[10px]">{idx + 1}</span>
                            </div>

                            <div className="text-center px-4">
                                <h3 className="font-serif text-2xl font-light mb-2">{step.title}</h3>
                                <p className="text-sm font-medium text-gray-800 mb-6">{step.sub}</p>
                                <p className="text-gray-500 text-sm leading-relaxed keep-all">
                                    {step.desc}
                                </p>
                            </div>
                        </div>
    ...
    return (
        <section className="bg-white pb-20">
            <div className="text-center py-24 reveal">
                <p className="text-xs tracking-[0.3em] uppercase text-gray-400 mb-4">Solution</p>
                ...
                <div key={index} className={`grid grid-cols-1 lg:grid-cols-2 min-h-[60vh] reveal`}>
                    ...
                    <div className={`flex items-center bg-white ${sol.reverse ? 'lg:order-1' : ''}`}>
                        <div className="p-10 lg:p-16 xl:p-20">
                            ...
                            >
                                <span className="border-b border-black pb-1">자세히 보기</span>
...
function CTASection() {
    return (
        <section className="py-32 bg-white text-black border-t border-gray-100">
            <div className="max-w-4xl mx-auto px-6 text-center reveal">
                <p className="text-xs tracking-[0.3em] uppercase text-gray-400 mb-6">Start Now</p>
                ...
                <Link
                    to="/services"
                    className="inline-block px-14 py-5 bg-black text-white text-sm tracking-[0.2em] uppercase hover:bg-gray-800 transition-all duration-300"
                >
                    서비스 보러가기
                </Link>
            </div>
